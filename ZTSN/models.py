...
class Subsession(BaseSubsession):
    ...
    def get_config_multivalue(self, value_name):
        ...
        if isinstance(parsed_value, list):
            ...
            #assert(len(parsed_value) == 1), value_name + f'{value_name} must contain exactly 1 value!'
    ...
    def get_timeseries_values(self):
        ...
        if 'news' in rows[0].keys():
            ...
            news = '' * len(prices)
        if 'risk' in rows[0].keys():
            risk =  [dic['risk'] if dic['risk'] else '' for dic in rows]
        else:
            risk = '' * len(prices)
        return asset, prices, news, risk
                
class Group(BaseGroup):
    pass

class Player(BasePlayer):
    cash = models.FloatField(initial=120)
    ...
    def set_payoff(self):
        ...
        if self.participant.vars['exp_to_pay'] == 1 and self.round_number == self.participant.vars['round_to_pay']:
            self.payoff = self.portfolio_value if self.portfolio_value > 0 else 0
            self.participant.vars['exp1_pay_ecu'] = self.payoff
